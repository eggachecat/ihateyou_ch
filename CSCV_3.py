...
def backTest(nDays=0, factor=0):
    ...
    holdingPeriod, sigma, stopLoss, length = 20, 1, 10, 100
    # 2) Prepare series
...
def generate_stock_price_with_normal(N, S_0=1):
    ...
    for i in range(N):
        ...
        stock_price_tsa.append(r * stock_price + S_0 * noise_ts[i])
        static_log_price_ts.append(r * static_log_price_ts[-1])
...
def run_methods():
    ...
    print(series)
    plt.plot(series)
    stock_price_tsa, static_log_price_ts = generate_stock_price_with_normal(length)
    plt.plot(static_log_price_ts, color="green")

    plt.plot(stock_price_tsa, color="red")

    plt.show()
    exit()


def compare_backTest(nDays=0, factor=0):
    # 1) Input parameters --- to be changed by the user
    holdingPeriod, sigma, stopLoss, length = 20, 1, 10, 100
    # 2) Prepare series
    date_, dates = dt.date(year=2000, month=1, day=1), []
    ...
    for i in range(series.shape[0]):
        ...
        if numBusinessDays(pDay_, dates[i]) <= nDays:
            series[i] += sigma * factor
    series, static_log_price_ts = generate_stock_price_with_normal(length)
    ...
    for i in range(len(series)):
        ...
        if truth > predict:
            ...
            if position == 1:
                position = 0
                bench_mark_pnl.append(truth - in_price)

    canvas = BTCanvas(shape=(2, 1))
    canvas.draw_line_chart_2d(list(range(len(series))), series, sub_canvas_id=2)
    canvas.draw_line_chart_2d(list(range(len(series))), static_log_price_ts, sub_canvas_id=2)

    canvas.draw_line_chart_2d(list(range(len(bench_mark_pnl))), np.cumsum(bench_mark_pnl))

    dynamic_ax = canvas.draw_line_chart_2d(range(5), range(5))

    equity_curve = dynamic_ax.lines[-1]
    canvas.show(1)
    equity_curve.set_ydata(range(10, 15))
    canvas.show(1)
    equity_curve.set_ydata(range(15, 20))
    canvas.show(1)
    canvas.freeze()
    exit()
